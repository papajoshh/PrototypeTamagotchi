...
export class Pet {
    constructor() {
        ...
        // Decoración actual
        this.currentRoom = 'style1'; // Por defecto style1
    ...
    update(deltaTime) {
        if (this.stage === LifeStage.Egg || this.stage === LifeStage.Dead)
            return;
    ...
    mixPersonalities(current, dominantMemory, fromStage) {
    ...
    feed(satiationStars, personalityType) {
        // Verificar si necesita generar recuerdo ANTES de saciar
        const wasNotFullySatiated = !this.hunger.isFullySatiated();
        this.hunger.satiate(satiationStars);
    ...
    play(personalityType, scorePercentage = 0) {
    ...
    generateIngredientRewards(personalityType, scorePercentage) {
        const rewards = [];
    ...
    createIngredientByType(personalityType, tier) {
