...
export class TheButtonRewards {
    constructor(canvas, ingredientCache) {
        this.rewards = [];
    ...
    async show(tier1, tier2, tier3, personality) {
        this.rewards = [];
    ...
    createIngredient(personality, tier) {
    ...
    render() {
        if (!this.isAnimating && this.rewards.length === 0)
            return;
        this.ctx.save();
        const now = Date.now();
