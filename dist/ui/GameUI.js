...
export class GameUI {
    constructor(canvas, pet, settings) {
        ...
        this.FEEDBACK_DURATION = 2000; // 2 segundos en ms
        // Sprite loading
        this.spriteCache = new Map();
        this.roomCache = new Map();
        this.mochiCache = new Map();
        this.ingredientCache = new Map();
        this.personalityIconCache = new Map();
        this.styleIconCache = new Map();
        // Estado de la interfaz de comida
        this.selectedIngredient = null; // ID del ingrediente seleccionado (solo uno)
        this.previewMochi = null;
        this.ingredientScrollX = 0; // Scroll horizontal del grid de ingredientes
        this.isDraggingIngredients = false;
        this.dragStartX = 0;
        this.dragStartScrollX = 0;
        // Estado de la interfaz de minijuegos
        this.selectedMinigame = 'theButton'; // ID del minijuego seleccionado (default: El Botón)
        this.minigames = [
    ...
    handleMenuPanelClick(x, y, panelY) {
        ...
        if (this.currentMenu === 'feed') {
            ...
            const checkIngredientClick = (item, col, row, baseX) => {
            ...
            for (let i = 0; i < allTier1.length; i++) {
                ...
                const row = i % rows;
                if (checkIngredientClick(allTier1[i], col, row, currentX))
                    return;
            }
            ...
            for (let i = 0; i < tier2Items.length; i++) {
                const col = Math.floor(i / rows);
                const row = i % rows;
                if (checkIngredientClick(tier2Items[i], col, row, currentX))
                    return;
            }
            ...
            for (let i = 0; i < tier3Items.length; i++) {
                const col = Math.floor(i / rows);
                const row = i % rows;
                if (checkIngredientClick(tier3Items[i], col, row, currentX))
                    return;
            }
        }
    ...
    renderIngredientSelector() {
        ...
        inventory.forEach((item, index) => {
            ...
            this.ctx.fillText(`${item.ingredient.name} ${tierStars}`, panelX + 30, itemY + 20);
            // Cantidad
            this.ctx.fillStyle = '#000';
            this.ctx.font = '12px Arial';
            this.ctx.fillText(`x${item.quantity} | ${item.ingredient.personality}`, panelX + 30, itemY + 38);
            this.ctx.font = '14px Arial';
            // Borde
            this.ctx.strokeStyle = '#000';
            this.ctx.strokeRect(panelX + 20, itemY, panelWidth - 40, itemHeight - 5);
    ...
    renderPet() {
        ...
        if (sprite && sprite.complete && sprite.naturalHeight > 0) {
            ...
            this.ctx.drawImage(sprite, centerX - renderWidth / 2, centerY - renderHeight / 2, renderWidth, renderHeight);
        }
        else {
            // Fallback to emojis
    ...
    renderGrowthBar() {
        if (this.pet.stage === LifeStage.Egg || this.pet.stage >= LifeStage.ReadyToAscend)
            return;
        const barX = 40;
        const barY = 520;
    ...
    renderActionButtons() {
        ...
        const buttonHeight = 60;
        const buttons = [
            { id: 'feed', label: '🍙', name: 'Alimentar' },
            { id: 'play', label: '🎮', name: 'Jugar' },
            { id: 'room', label: '🏠', name: 'Decoración' },
        ];
    ...
    renderStateIndicators() {
        ...
        if (this.pet.stage === LifeStage.Egg || this.pet.stage === LifeStage.Dead)
            return;
    ...
    renderFeedContent(startY) {
        ...
        const renderIngredient = (item, x, row) => {
            const cellX = x;
            const cellY = gridStartY + row * (cellSize + cellGap);
    ...
    async handleMinigameEnd(scorePercentage, personality) {
        ...
        rewards.forEach(reward => {
            if (reward.tier === 1)
                rewardCounts.tier1++;
            else if (reward.tier === 2)
                rewardCounts.tier2++;
            else if (reward.tier === 3)
                rewardCounts.tier3++;
        });
