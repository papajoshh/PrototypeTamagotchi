...
export class SettingsUI {
    ...
    renderContent(x, y, width, availableHeight, scrollOffset) {
    ...
    calculateContentHeight(width) {
        ...
        totalHeight += 30 + 15; // Header
        totalHeight += 35 + 10 + 35 + 35 + 35; // Toggle general + línea + 3 toggles
        totalHeight += 15 + 15; // Padding
        return totalHeight;
    }
    renderSettingsContent(x, y, width, scrollOffset) {
        this.ctx.save();
        let currentY = y;
        // ========== Sección MASCOTA ==========
        currentY = this.renderSettingsSection(x, currentY, width, 'Mascota', (sectionX, sectionY) => {
            ...
            offsetY += this.renderSettingRow(sectionX + 15, sectionY + offsetY, width - 30, '😴 Dormir Automático', this.settings.sleep.isAutomatic, scrollOffset, 'sleep-auto');
            // Botón: Horario de sueño
            offsetY += this.renderSettingButton(sectionX + 15, sectionY + offsetY, width - 30, 'Horario de sueño', '⏰', scrollOffset, 'sleep-schedule');
            // Aviso: Solo si dormir automático está desactivado
            if (!this.settings.sleep.isAutomatic) {
                offsetY += 10; // Espacio antes del aviso
                offsetY += this.renderWarningBox(sectionX + 15, sectionY + offsetY, width - 30, 'CUIDAO', 'Si desactivas el dormir automático tendrás que apagar y prender la luz manualmente con el botón de mimir del menu principal para que tu mascota duerma y no se quede desatendida durante la noche.');
            }
            return offsetY;
        });
        currentY += 15;
        ...
        currentY = this.renderSettingsSection(x, currentY, width, 'Sonido', (sectionX, sectionY) => {
            ...
            offsetY += this.renderSettingRow(sectionX + 15, sectionY + offsetY, width - 30, 'Música', this.settings.audio.musicEnabled, scrollOffset, 'music-enabled');
            // Slider de música (SIEMPRE visible, como en Figma)
            offsetY += this.renderSettingSlider(sectionX + 15, sectionY + offsetY, width - 30, this.settings.audio.musicVolume, scrollOffset, 'music-volume');
            return offsetY;
        });
        currentY += 15;
        // ========== Sección VIBRACIÓN ==========
        currentY = this.renderSettingsSection(x, currentY, width, 'Vibración', (sectionX, sectionY) => {
            return this.renderSettingRow(sectionX + 15, sectionY, width - 30, 'Vibración', this.settings.audio.vibrationEnabled, scrollOffset, 'vibration-enabled');
        });
        currentY += 15;
        ...
        currentY = this.renderSettingsSection(x, currentY, width, 'Notificaciones', (sectionX, sectionY) => {
            let offsetY = 0;
            // Toggle general
            offsetY += this.renderSettingRow(sectionX + 15, sectionY + offsetY, width - 30, 'Activar/Desactivar todas', this.settings.notifications.allEnabled, scrollOffset, 'notif-all');
            // Línea divisoria
            this.ctx.strokeStyle = '#000';
            ...
            offsetY += this.renderSettingRow(sectionX + 15, sectionY + offsetY, width - 30, 'Hambre', this.settings.notifications.hungerEnabled, scrollOffset, 'notif-hunger');
            offsetY += this.renderSettingRow(sectionX + 15, sectionY + offsetY, width - 30, 'Enfermedad', this.settings.notifications.illnessEnabled, scrollOffset, 'notif-illness');
            offsetY += this.renderSettingRow(sectionX + 15, sectionY + offsetY, width - 30, 'Evolución próxima', this.settings.notifications.evolutionEnabled, scrollOffset, 'notif-evolution');
            return offsetY;
        });
        this.ctx.restore();
        return currentY;
    ...
    renderSettingsSection(x, y, width, title, renderContent) {
        const padding = 15;
    ...
    renderSettingRow(x, y, width, label, isOn, scrollOffset, id) {
        const rowHeight = 35;
        // Label
        this.ctx.fillStyle = '#000';
    ...
    renderSettingButton(x, y, width, label, icon, scrollOffset, id) {
    ...
    // Renderiza un slider
    renderSettingSlider(x, y, width, value, scrollOffset, id) {
    ...
    renderWarningBox(x, y, width, title, text) {
        const padding = 15;
        const cornerRadius = 10;
        const lineHeight = 16;
    ...
    handleSleepSchedulePopupClick(x, y) {
        const popupW = 360;
        const popupH = 380;
        const popupX = (this.canvas.width - popupW) / 2;
        const popupY = (this.canvas.height - popupH) / 2;
    ...
    handleHourPickerClick(x, y, centerY, onHourChange) {
