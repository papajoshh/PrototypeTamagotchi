// EdgyBunBun Minigame - UI Rendering
...
export class EdgyBunBunUI {
  ...
  private stunPlatformSprite: HTMLImageElement | null = null;
  private softPlatformSprite: HTMLImageElement | null = null;
  ...
  // Estado visual
  private jumpAnimationProgress: number = 0;
  private fallAnimationProgress: number = 0;
  ...
  private previousX: Side = 'left';

  // Cámara suave
  private cameraY: number = 0; // Posición actual de cámara (interpolada)
  ...
  private jump(side: Side): void {
    const state = this.game.getState();
    if (!state.bunBun.canMove || this.isJumping || this.isFalling) return;
  ...
  private renderPlayingScreen(state: EdgyBunBunGameState): void {
    this.ctx.save();
  ...
  private renderGameArea(state: EdgyBunBunGameState): void {
  ...
  private renderPlatform(x: number, y: number, w: number, h: number, type: string, side: 'left' | 'right'): void {
  ...
  private renderBunBun(state: EdgyBunBunGameState, minLevel: number, gameAreaY: number, gameAreaHeight: number, platformSpacing: number): void {
    const maxHeight = 80;
    const platformWidth = 150;
    const platformHeight = 40;

    // Determinar dimensiones (sprite o emoji)
    let petWidth: number, petHeight: number;
    if (this.petSprite && this.petSprite.complete) {
      const aspectRatio = this.petSprite.width / this.petSprite.height;
      petHeight = maxHeight;
  ...
  private renderFinishedScreen(state: EdgyBunBunGameState): void {
    this.ctx.save();
