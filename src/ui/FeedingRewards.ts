...
export class FeedingRewards {
  ...
  private stars: FloatingStar[] = [];
  private memories: FloatingMemory[] = [];
  ...
  async show(ingredient: Ingredient, memoryText?: string): Promise<void> {
    this.stars = [];
    this.memories = [];
  ...
  async showMemoryOnly(memoryText: string): Promise<void> {
    this.stars = [];
    this.memories = [];
    this.isAnimating = true;
  ...
  private addFloatingMemory(text: string): void {
    const centerX = this.canvas.width / 2;
  ...
  render(): void {
    if (!this.isAnimating && this.stars.length === 0 && this.memories.length === 0) return;
